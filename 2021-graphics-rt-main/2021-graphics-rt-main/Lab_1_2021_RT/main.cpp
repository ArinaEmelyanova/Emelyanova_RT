...
{
  auto  background_color = float3(240 / float(255), 1.0f, 1.0f);
...
void create_scene()
{
  ...
  auto sphere1= std::make_shared<Sphere>(float3(1.0f, 5.5f, +4.0f), 5.5, new IdealMirror(float3(0.0f, 128/ float(255), 128/ float(255))));
  auto sphere2= std::make_shared<Sphere>(float3(3.0f, 2.0f, +12.0f), 2.0, new Diffuse_material(float3(127/ float(255), 1.0f, 212 / float(255))));

  auto triangle = std::make_shared<Triangle>(float3(2.0f, 10.0f, 6.0f), float3(5.0f, 15.0f, 4.0f),float3(8.0f, 10.0f, 5.0f), new Diffuse_material(float3(153/ float(255), 1.0f, 204 / float(255))));

  auto square  = std::make_shared<Square >(float3(-12.0f, 9.0f, 1.0f),6.0,new Diffuse_material(float3(153 / float(255), 204 / float(255), 0 / float(255))));
  

  auto  parallelepiped = std::make_shared<Parallelepiped>(float3(-16.0f, -0.0f, -10.0f), float3(-9.0f, +8.0f, 2.0f), new IdealMirror(float3(220 / float(255), 220 / float(255), 220 / float(255))));
  

  // освещение
  auto light1 = std::make_shared<Lighting>(float3(15.0f, 10.0f, 10.0f), float3(1.0f, 1.0f, 1.0f));
  auto light2 = std::make_shared<Lighting>(float3(-10.0f, 25.0f, 10.0f), float3(1.0f, 1.0f, 1.0f));
